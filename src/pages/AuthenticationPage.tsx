...
const AuthenticationPage: React.FC = () => {
	...
	const handleGoogleSignIn = async () => {
		...
		try {
			...
			console.log("🔐 AUTH_PAGE: Google sign in successful:", {
				uid: user.uid,
				email: user.email,
			});
	...
	const onSubmit = async (data: AuthFormData) => {
		console.log("🔐 AUTH_PAGE: Form submit clicked", {
			isSignUp,
			email: data.email,
		});
		try {
			...
			console.log("🔐 AUTH_PAGE: Email auth successful:", {
				uid: user.uid,
				email: user.email,
			});
			dispatch(setUser({ uid: user.uid, email: user.email || "" }));
								...
								<div className="absolute inset-0 bg-gradient-to-br from-orange-500 to-red-600 flex items-center justify-center">
									<div className="text-center text-white p-8">
										...
										<p className="text-white/80">
											...
											experiences to our participants." - Sofia Davis
										</p>
									</div>
								</div>
							</div>

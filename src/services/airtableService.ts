...
export const fetchUserByFirebaseUID = async (
	...
	try {
		...
		if (sortedRecords.length > 1) {
			console.warn(
				`⚠️  Found ${sortedRecords.length} duplicate records for UID ${uid}:`,
			);
			sortedRecords.forEach((record, index) => {
				console.log(
					`   ${index + 1}. ${record.id} - Group: ${record.fields.AssignedGroup} - Email: ${record.fields.Email} - Created: ${(record as any).createdTime}`,
				);
			});

			// Prefer the most recent record with onboarding completed
			const completedRecords = sortedRecords.filter(
				(r) => r.fields.OnboardingCompleted,
			);
			const selectedRecord =
				completedRecords.length > 0 ? completedRecords[0] : sortedRecords[0];

			console.log(
				`✅ Selected record: ${selectedRecord.id} (Group: ${selectedRecord.fields.AssignedGroup})`,
			);
		}

		const record =
			sortedRecords.length > 1
				? sortedRecords.filter((r) => r.fields.OnboardingCompleted)[0] ||
					sortedRecords[0]
				: sortedRecords[0];
		...
		console.log("User record found:", {
			...
			group: record.fields.AssignedGroup,
		});
		return {
			id: record.id,
...
export const upsertUser = async (userData: {
	...
	try {
		...
		if (existingUser) {
			console.log(
				"User exists, checking onboarding status:",
				existingUser.fields.OnboardingCompleted,
			);

			if (existingUser.fields.OnboardingCompleted) {
				console.log(
					"User already completed onboarding, returning existing record",
				);
				return {
					id: existingUser.id,
				...
				const updatedRecord = await base("Users").update(existingUser.id, {
					...
					...(existingUser.fields.AssignedGroup
						? {}
				...
				return {
					...
					createdTime:
						(updatedRecord as any).createdTime || existingUser.createdTime,
				};
			}
		} else {
...
// Content operations
export const fetchContentForGroup = async (
...
export const fetchUserResponses = async (
	...
	try {
		const records = await base("UserResponses")
			.select({
				filterByFormula: `FIND("${userRecordId}", ARRAYJOIN({User})) > 0`,
			})
			.all();
		return handleAirtableResponse(records);
	} catch (error) {

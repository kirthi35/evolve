...
function App() {
	...
	const { user, isAuthenticated, status, airtableStatus } = useAppSelector(
		(state) => state.user,
	);
	...
	useEffect(() => {
		...
		dispatch(checkAuthStatus())
			.then((authResult) => {
				console.log(
					"🎯 APP: checkAuthStatus completed with result:",
					authResult,
				);

				// Step 2: If authenticated, fetch Airtable data
				const authPayload = authResult.payload as {
					uid: string;
					email: string;
				} | null;
				if (authPayload && authPayload.uid) {
					console.log(
						"🔄 APP: User authenticated, now fetching Airtable data...",
					);
					return dispatch(fetchAirtableUserData(authPayload.uid));
				} else {
					console.log(
						"❌ APP: User not authenticated, skipping Airtable fetch",
					);
			...
			.then((airtableResult) => {
				if (airtableResult) {
					console.log(
						"🎯 APP: fetchAirtableUserData completed with result:",
						airtableResult,
					);
				}
			})
			.catch((error) => {
				console.error(
					"💥 APP: Auth or Airtable fetch failed with error:",
					error,
				);
			})
			.finally(() => {
	...
	console.log(
		"🖥️  APP: - authenticated + no record + airtable not done:",
		condition4,
	);
